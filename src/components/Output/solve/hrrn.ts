...
const calcRR = (
  ...
  return (currentTime - arrivalTime + burstTime) / burstTime;
};
...
export const hrrn = (arrivalTime: number[], burstTime: number[]) => {
  ...
  for (let i = 0; i < processesInfo.length; i++) {
    if (i === 0) {
      ...
      const currentTime = previousFinishTime;
      const rqSortedByBT = [...readyQueue].sort((a, b) => {
        if (calcRR(currentTime, a.at, a.bt) < calcRR(currentTime, b.at, b.bt)) return 1;
        if (calcRR(currentTime, a.at, a.bt) > calcRR(currentTime, b.at, b.bt)) return -1;
        if (a.at > b.at) return 1;
        if (a.at < b.at) return -1;
        return 0;
